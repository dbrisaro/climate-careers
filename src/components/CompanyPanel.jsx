...
function Stars({ n }) {
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((i) => (
        <span key={i} className={`text-base ${i <= n ? 'text-amber-400' : 'text-gray-200 dark:text-gray-700'}`}>
          ★
        </span>
      ))}
    </div>
  ...
  return (
    <aside className="w-full md:w-80 lg:w-96 border-l border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950 flex flex-col overflow-y-auto shrink-0">
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-100 dark:border-gray-800">
        <span className="text-xs text-gray-400 font-medium uppercase tracking-wide">Fit</span>
        <Stars n={company.fit} />
